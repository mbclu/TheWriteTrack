...
When(/^I touch the Next Button I see the "(.*?)" view$/) do |letter|
  ...
  x_touch_point = letter_rect["width"] - 20 # There is ten pixel padding from the right, go a little more
...
When(/^I touch the Previous Button I see the "(.*?)" view$/) do |letter|
  next_letter = (letter.codepoints.first + 1).chr
  wait_for_elements_exist(["view marked:'#{next_letter}'"], {:timeout => 5})
  letter_view = query("view marked:'#{next_letter}'")[0]
  letter_rect = letter_view["rect"]
  x_touch_point = letter_rect["x"] + 20 # There is ten pixel padding from the left, go a little more
  y_touch_point = letter_rect["center_y"]
  sleep 1.0 	# there is a transition, 
  				# so even though we could see the screen before this point, 
  				# we need to wait for the transition to finish before we try and press the button
